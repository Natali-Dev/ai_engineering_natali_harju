...
async def filter_books(
    start_year: int = Query(default=1950, gt = 1500, lt=CURRENT_YEAR +1,
                            description = "Filters books that are newer then this year"),
    author: str = Query(None, description="Filter by first and last name")
):
    filtered_books = [book for book in books if start_year < book.year]
    if author: 
        filtered_books = [
            book for book in filtered_books if author.casefold() == book.author.casefold()
        ]
